...
<header class="flex flex-col items-start gap-2">
	<span class="text-accent text-sm font-bold">{data.group}</span>
	<h1 class="text-3xl font-extrabold sm:text-4xl">{data.md?.title ?? data.title}</h1>
	{#if data.md?.description}
		<p class="text-muted-foreground sm:text-lg">{data.md.description}</p>
	{/if}
	<div
